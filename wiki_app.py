...
def propositions( db ):
    ...
    if DATA_DEBUG:
            print 'search_propositions: BEFORE'
    propositions = search_propositions( query, lang )
    if DATA_DEBUG:
            print 'search_propositions: AFTER'
    if DATA_DEBUG:
        print 'is_query_cached: BEFORE'
    is_cached = is_query_cached( db, query, lang )
    if DATA_DEBUG:
            print 'is_query_cached: AFTER'
    return {
...
def data( db ):
    ...
    if cached:
        if DATA_DEBUG:
            print 'get_cached_data: BEFORE'
        data = get_cached_data( db, query, lang )
        if DATA_DEBUG:
            print 'get_cached_data: AFTER'
    else:
        if DATA_DEBUG:
            print 'get_fresh_data: BEFORE'
        data = get_fresh_data( db, query, lang )
        if DATA_DEBUG:
            print 'get_fresh_data: AFTER'
...
def get_query_id( db, query, lang ):
    db_query = '''select query_id from id_map
                  where query=? and lang=?'''
    query_id = db.execute( db_query, (query, lang) ).fetchone()
    
    return query_id if query_id is None else query_id[0]
    
def get_or_create_query_id( db, query, lang ):
    query_id = get_query_id( db, query, lang )
    if query_id is None:
        new_query_id = get_max_query_id( db, query, lang ) + 1
        db_query = '''insert into id_map
                      values (?,?,?)'''
...
def get_max_query_id( db, query, lang ):
    min_id = 0
    db_query = '''select query_id from id_map'''
    ids = db.execute( db_query, () ).fetchall()
    print 'ids:', ids
    return max( ids )[0] if len( ids ) > 0 else min_id
...
# res = (year, month, count)
# def sort_fun( res1, res2 ):
    # if res1[0] != res2[0]:
        # return res1[0] - res2[0]
    # else:
        # return res1[1] - res2[1]
    
def is_query_cached( db, query, lang ):
    id = get_query_id( db, query, lang )
    return id is not None
    
# TODO: maybe use?
def update_search_data( db, query, lang, data):
    pass
